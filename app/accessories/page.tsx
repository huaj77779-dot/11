        ...
        <section className="accessory-catalog">
          ...
          <div className="accessory-grid">{visibleStyles.map(({ key, product, sku }) => {
            ...
            return <article key={key} className="accessory-card">
              <div className="accessory-photo">{sku.imageUrl || product.imageUrl ? <img src={sku.imageUrl || product.imageUrl || ""} alt={`${product.title} ${skuName}`} loading="lazy" referrerPolicy="no-referrer" /> : <span>◇</span>}{sku.imageFallback && <em>{zh ? "商品主图" : "Product image"}</em>}</div>
